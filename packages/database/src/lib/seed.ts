...
async function seed() {
	if (!env.DEVELOPMENT_GUILD_ID || !env.DEVELOPMENT_WEBHOOK_ID || !env.DEVELOPMENT_WEBHOOK_URL)
	...
	await guildWebhookModel.create([
		{ guildId: env.DEVELOPMENT_GUILD_ID, id: env.DEVELOPMENT_WEBHOOK_ID, webhook: env.DEVELOPMENT_WEBHOOK_URL, type: "text" }
	]);
